...
<li><em>Export save</em> downloads the save file; <em>Import save</em> loads one, also a save from the Mac version (<code>save/save/501Name</code>: user number, then the character name).</li>
...
parts.append(section('about', 'About the game',
                     guide.pop('How Hack differs from Angband')))
...
parts.append('<h2 id="h-version">About this version</h2><ul>'
             '<li>Based on <strong>restoHack</strong> (<a href="https://github.com/Critlist/restoHack">github.com/Critlist/restoHack</a>), '
             "a restoration of Hack 1.0.3 (1985) by Jay Fenlason and Andries Brouwer.</li>"
             '<li>Our changes (tiled window, auto-explore, stairs walking, command menu, inventory cursor and item menus, web build) '
             'are local to this port.</li>'
             '<li>Tiles: DawnLike by DragonDePlatino and DawnBringer (CC-BY 4.0); NetHack 3.6 tiles (NetHack General Public License).</li></ul>')
print('\n'.join(parts))
