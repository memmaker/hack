...
struct obj *steal(struct monst *mtmp) {
  struct obj *otmp;
  int tmp;
  int named = 0;
  ...
  if (!invent) {
    if (Blind)
      ...
      pline("%s tries to rob you, but she finds nothing to steal!",
            Monnam(mtmp));
    return (NULL); /* let her flee - no object stolen */
  }
  tmp = 0;
  for (otmp = invent; otmp; otmp = otmp->nobj)
    if (otmp != uarm2)
  ...
  if ((otmp->owornmask & (W_ARMOR | W_RING))) {
    ...
    case ARMOR_SYM:
      ...
      }
      {
        ...
        pline("%s seduces you and %s off your %s.",
              ...
              otmp->cursed ? "helps you to take" : "you start taking",
              (otmp == uarmg)   ? "gloves"
              : (otmp == uarmh) ? "helmet"
                                : "armor");
        named++;
    ...
    default:
      impossible("Tried to steal a strange worn thing.", 0, 0);
    }
  } else if (otmp == uwep)
    setuwep((struct obj *)0);
  if (otmp->olet == CHAIN_SYM) {
    impossible("How come you are carrying that chain?", 0, 0);
  }
  ...
  mpickobj(mtmp, otmp);
  return ((multi < 0) ? (struct obj *)0 : otmp);
}
