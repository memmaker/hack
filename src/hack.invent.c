...
#include "hack.h"
#include <stdio.h>
extern struct obj *splitobj(struct obj *otmp, int cnt);
...
void freeinv(obj) struct obj *obj;
{
  struct obj *otmp;
...
void delobj(obj) struct obj *obj;
{
  freeobj(obj);
  unpobj(obj);
  obfree(obj, (struct obj *)0);
...
void freeobj(obj) struct obj *obj;
{
  struct obj *otmp;
...
void freegold(gold) struct gold *gold;
{
  struct gold *gtmp;
...
void deltrap(trap) struct trap *trap;
{
  struct trap *ttmp;
 ...
 */
int askchain(objchn, olets, allflag, fn, ckfn, max)
struct obj *objchn;
char *olets;
int allflag;
...
int max;
{
  ...
  for (otmp = objchn; otmp; otmp = otmp2) {
    if (ilet == 'z')
      ...
      ilet++;
    otmp2 = otmp->nobj;
    if (olets && *olets && !index(olets, otmp->olet))
      continue;
    if (ckfn && !(*ckfn)(otmp))
      continue;
    if (!allflag) {
...
char obj_to_let(obj) /* should of course only be called for things in invent */
struct obj *obj;
{
  struct obj *otmp;
  char ilet;
...
static char *xprname(struct obj *obj, char let) /* ANSI C conversion from K&R style */
{
  ...
  (void)snprintf(li, BUFSZ,
                 "%c - %s.", /* MODERN: Safe sprintf replacement - identical
                                output, prevents overflow */
                 flags.invlet_constant ? obj->invlet : let, doname(obj));
  return (li);
}
...
int dotypeinv() /* free after Robert Viduya */
...
int dolook() {
  struct obj *otmp,
      *otmp0 = NULL; /* MODERN: Initialize to prevent uninitialized use */
  struct gold *gold =
      NULL; /* MODERN: Initialize to prevent uninitialized use */
  const char *verb =
      Blind ? "feel"
            : "see"; /* MODERN: const because assigned string literals */
  int ct = 0;
  ...
  if (gold) {
    ...
    (void)snprintf(gbuf, sizeof(gbuf),
                   "%ld gold piece%s", /* MODERN: Safe sprintf replacement -
...
void stackobj(obj) struct obj *obj;
{
